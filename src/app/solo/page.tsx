...
export default function SoloGame() {
  ...
  const spawnWave = useCallback(() => {
    ...
    const newStones: Stone[] = lanes.map((lane, index) => ({
      ...
      lane,
      y:         -STONE_SIZE - 20,
      value:     lane === answerLane ? eq.answer : decoys[index < decoys.length ? index : decoys.length - 1],
  ...
  }, [countdown, gamePhase, gameLoop, updateStreak]);
  ...
  const getPopStyle = (type: RewardPop["type"]) => {
    if (type === "gem")   return { color: "#C4AEFF" };
    if (type === "coins") return { color: "#FFE066" };
    if (type === "wrong") return { color: "#E85454" };
    return { color: "#F5C842" };
  };
  ...
  return (
    ...
    >
      <div className="absolute top-[4%] left-[3%] opacity-80 pointer-events-none z-10"><Cloud size="lg" /></div>
      <div className="absolute top-[8%] right-[5%] opacity-70 pointer-events-none z-10"><Cloud size="md" /></div>
      <div className="absolute top-[3%] left-[40%] opacity-60 pointer-events-none z-10"><Cloud size="sm" /></div>

      {/* TOP BAR */}
      <div style={{ background: "rgba(26,26,46,0.88)", backdropFilter: "blur(8px)", padding: "10px 20px 8px", zIndex: 20, flexShrink: 0 }}>

        {/* Timer bar */}
        <div style={{ background: "#333", borderRadius: 99, height: 10, overflow: "hidden", marginBottom: 10 }}>
          <div style={{ width: `${timerWidth}%`, height: "100%", borderRadius: 99, background: timerColor, transition: "background 0.5s ease" }} />
        </div>

        {/* Equation and stats */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontFamily: "var(--font-game)", fontSize: "clamp(1.4rem, 4vw, 2rem)", color: "#F5A623", letterSpacing: 1, textShadow: "0 2px 8px rgba(0,0,0,0.3)" }}>
            {equation.question}
          </div>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <div
              style={{
                background: coinLost ? "rgba(232,84,84,0.25)" : rewardFlash === "coin" || rewardFlash === "coins" ? "rgba(245,200,66,0.35)" : "rgba(245,200,66,0.18)",
                borderRadius: 99,
                padding: "4px 12px",
                display: "flex",
                alignItems: "center",
                gap: 5,
                border: coinLost ? "1.5px solid #E85454" : rewardFlash === "coin" || rewardFlash === "coins" ? "1.5px solid #FFD700" : "1.5px solid transparent",
                transition: "all 0.3s ease",
                boxShadow: rewardFlash === "coin" || rewardFlash === "coins" ? "0 0 12px rgba(255,215,0,0.5)" : "none",
              }}
            >
              <CoinSVG size={16} />
              <span style={{ fontFamily: "var(--font-game)", fontSize: 14, color: coinLost ? "#E85454" : "#F5C842", transition: "color 0.3s ease" }}>
                {sessionCoins}
              </span>
            ...
            >
              <GemSVG size={14} />
              <span style={{ fontFamily: "var(--font-game)", fontSize: 14, color: gemFlash ? "#C4AEFF" : "#9B7FE8", transition: "color 0.3s ease" }}>
                {sessionGems}
              </span>
              {gemFlash && <span style={{ fontFamily: "var(--font-game)", fontSize: 11, color: "#C4AEFF" }}>+1!</span>}
            </div>

            {/* Level badge */}
            <div style={{ background: "rgba(123,111,232,0.2)", borderRadius: 99, padding: "4px 12px" }}>
              <span style={{ fontFamily: "var(--font-game)", fontSize: 14, color: "#A89FF5" }}>Lv {level}</span>
            </div>
      ...
      >
        ...
        {gamePhase === "countdown" && (
          <div style={{ position: "absolute", inset: 0, background: "rgba(26,26,46,0.75)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
            <div style={{ fontFamily: "var(--font-game)", fontSize: "clamp(5rem, 20vw, 8rem)", color: "#F5A623", textShadow: "0 6px 0 #C47A10" }}>
        ...
        {showHint && (
          <div style={{ position: "absolute", inset: 0, background: "rgba(26,26,46,0.92)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 50, padding: "24px 20px" }}>
            <div style={{ background: "rgba(123,111,232,0.2)", border: "2px solid #7B6FE8", borderRadius: 20, padding: "16px 20px", marginBottom: 16, maxWidth: 480, width: "100%", textAlign: "center" }}>
              <div style={{ fontFamily: "var(--font-game)", fontSize: 14, color: "#A89FF5", marginBottom: 8 }}>did you know?</div>
              <div style={{ fontFamily: "var(--font-ui)", fontSize: 15, color: "white", lineHeight: 1.6 }}>{HINT_FACTS[hintIndex].fact}</div>
            </div>
            <div style={{ background: "rgba(93,190,74,0.15)", border: "2px solid #5DBE4A", borderRadius: 20, padding: "14px 20px", marginBottom: 24, maxWidth: 480, width: "100%", textAlign: "center" }}>
              <div style={{ fontFamily: "var(--font-game)", fontSize: 14, color: "#5DBE4A", marginBottom: 6 }}>hint for {equation.question}</div>
              <div style={{ fontFamily: "var(--font-ui)", fontSize: 15, color: "white", lineHeight: 1.6 }}>{HINT_FACTS[hintIndex].hint}</div>
            </div>
            <button
              onClick={() => {
                setShowHint(false);
              ...
              style={{ background: "linear-gradient(to bottom, #FFD700, #F5A623)", border: "none", borderRadius: 99, padding: "12px 36px", fontFamily: "var(--font-game)", fontSize: 16, color: "white", cursor: "pointer", boxShadow: "0 4px 0 #C47A10" }}
            >
              got it — back to game!
            </button>
          </div>
